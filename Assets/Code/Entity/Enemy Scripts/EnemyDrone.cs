...
public class EnemyDrone : Enemy
{
    ...
    new void Update()
    {
        ...
        if (UnityEngine.Random.Range(0, 100) == 0)
        {
            ...
            {
                moveDirection = new Vector3(0, 0, 0); // sometimes it moves
            }
            else
            {
                float angle = Mathf.Asin(UnityEngine.Random.Range(-1f, 1f)); // up down
                float theta = UnityEngine.Random.Range(-Mathf.PI, Mathf.PI); // xz plane
                moveDirection = new Vector3(Mathf.Cos(angle) * Mathf.Cos(theta), Mathf.Sin(angle), Mathf.Cos(angle) * Mathf.Cos(theta));
            }
        }
